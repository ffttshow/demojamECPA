  ...
  var Surface=function(node){
    ...
    var renderSurface=function(){
      ...
      for(var x=0;x<xlength-1;x++){
        for(var y=0;y<ylength-1;y++){
          ...
          d0.push({
            ...
            depth: depth, point: [(data[x][y][0]+displayWidth/2).toFixed(10), (data[x][y][1]+displayHeight/2).toFixed(10)], data: originalData[x][y]
          });
          /*if (!d1[x].length) {
              d1[x].push([data[x][y][0]+displayWidth/2, data[x][y][1]+displayHeight/2]);
              d1px = - data[x][y][0];
              d1py = - data[x][y][1];
          }*/
        }
      ...
      for(var x=0;x<xlength-1;x++){
    	  d1[x] = d1[x] || [];
          for(var y=0;y<ylength;y++){
        	  d1[x].push([(data[x][y][0]+displayWidth/2).toFixed(10), (data[x][y][1]+displayHeight/2).toFixed(10)]);
          }
      }
      ...
      //Start to draw edge
      dr = node.selectAll('polyline').data(d1);
      dr.enter().append('polyline');
      dr.attr("fill","none").attr("stroke", "blue").attr("stroke-width",2)
